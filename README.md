...
* ```Planck```:
  ...
  * ```Planck lowT```:
    likelihood based on the PR3 low-ℓ TT posterior distributions of measured Cℓ fitted with a Wishart ($\alpha_\ell, \beta_\ell$). We then rescaled to the mock fiducial model with $\beta_\ell \longrightarrow \beta_\ell^{\rm mock} = C_{\ell,{\rm mock}}^{TT}\,(\alpha_\ell+1)$. Multipole range available $\ell \in [2,29]$.
